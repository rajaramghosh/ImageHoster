...
public class UserController {
    ...
    public String registerUser(User user, RedirectAttributes redirectAttr) {
        // Register the user only if the password contains 1 alphabet, 1 number,
        // and 1 special character or else send the error message
        if(checkIfPasswordHasRequiredStrength(user.getPassword())) {
            userService.registerUser(user);
            return "users/login";
        } else {
            String error = "Password must contain atleast 1 alphabet, 1 number & 1 special character";
            redirectAttr.addAttribute("passwordTypeError", error).addFlashAttribute("passwordTypeError", error);
    ...
    // Check if the password contain at least 1 alphabet (a-z or A-Z), 1 number (0-9) and
    // 1 special character (any character other than a-z, A-Z and 0-9) and return true in
    // that case or else return false
    private Boolean checkIfPasswordHasRequiredStrength(String password){
